...
export class Volunteer {
    ...
    birthDate: string;
    @Column({ type: 'varchar', length: 30, nullable: false,name: 'phone_number' })
    phoneNumber: string;
    @Column({ type: 'varchar', length: 100, nullable: true, name: 'email' })
    email: string;
    @Column({ type: 'varchar', length: 100, nullable: true, name: 'num_identification' })
    numIdentification: string;
    //was_voluntary? a que se refier?

    @Column({ type: 'boolean',nullable: false, default: true, name: 'is_voluntary' })
    isVoluntary: boolean;
